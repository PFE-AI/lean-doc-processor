...
class DataGenerator:
    ...
    def create_directory_structure(self):
        ...
        directories = [
            # Original structure
            os.path.join(self.output_dir, 'bill_payments_a4'),
            os.path.join(self.output_dir, 'bill_payments_80mm'),
            os.path.join(self.output_dir, 'commission_statements'),
            
            # New structure for pipeline
    ...
    def convert_pdf_to_images(self):
        """TODO 1: Convert all PDFs to images"""
        logger.info("------------------------------------------------------------------------------------")
        ...
        for folder in pdf_folders:
            pdf_dir = os.path.join(self.output_dir, folder)
    ...
    def augment_images(self):
        """TODO 2: Apply data augmentation techniques to images"""
        logger.info("------------------------------------------------------------------------------------")
    ...
    def preprocess_images(self):
        """TODO 3: Preprocess augmented images for training"""
        logger.info("------------------------------------------------------------------------------------")
        ...
        # Define standard sizes for different document types
        standard_sizes = {
            'bill_payments_a4': (512, 728),  # Aspect ratio similar to A4
            'bill_payments_80mm': (224, 448),  # Smaller for receipt format
            'commission_statements': (512, 728)  # Similar to A4
        }
    ...
    def copy_raw_data(self):
        """TODO 4: Copy original PDFs to raw data folder"""
        logger.info("------------------------------------------------------------------------------------")
        ...
        for folder in pdf_folders:
            source_dir = os.path.join(self.output_dir, folder)
    ...
    def prepare_training_data(self):
        """TODO 5: Prepare training, validation, and test datasets"""
        logger.info("------------------------------------------------------------------------------------")
        ...
        for i, folder in enumerate(processed_folders):
            ...
            for npy_file in npy_files:
                all_files.append(os.path.join(processed_dir, npy_file))
                all_labels.append(i)
    ...
    def process_all(self):
        """Process all bill payments and commission statements with complete pipeline"""
        logger.info("------------------------------------------------------------------------------------")
...
if __name__ == "__main__":
   ...
   # Processing Options
   parser.add_argument('--output-dir', type=str, default='generated', help='Output directory for all generated files')
   parser.add_argument('--page-size', type=int, default=1000, help='Page size for API pagination')
   parser.add_argument('--grayscale', action='store_true', default=True, help='Convert images to grayscale during preprocessing')
   parser.add_argument('--no-grayscale', dest='grayscale', action='store_false', help='Keep images in color during preprocessing')
   ...
   parser.add_argument('--skip-raw-backup', action='store_true', help='Skip raw data backup')
   ...
   try:
       ...
       logger.info("------------------------------------------------------------------------------------")
       if success:
           logger.info(f"INWI-REPORT-GENERATOR: Process completed successfully in {duration:.2f} seconds")
       else:
           logger.info(f"INWI-REPORT-GENERATOR: Process failed after {duration:.2f} seconds")
       logger.info("------------------------------------------------------------------------------------")
       ...
       if success and args.verbose:
           ...
           folders = [
               ('PDFs', ['bill_payments_a4', 'bill_payments_80mm', 'commission_statements']),
